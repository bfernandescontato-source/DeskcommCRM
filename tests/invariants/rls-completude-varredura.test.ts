...
const PROVA_PROPRIA: readonly Excecao[] = [
  ...
  { tabela: "campaign_group_events", razao: "tests/invariants/central-de-disparos-grupos.test.ts — dois tenants reais por JWT (countAs): o dono lê os avisos de grupo da própria campanha e a outra organização lê zero; o navegador só tem SELECT (INSERT/UPDATE/DELETE recusados a authenticated e anon); trilha append-only até para o service_role; escrita só por fn_campaign_record_group_event (service_role)" },
  ...
  { tabela: "campaign_message_versions", razao: "tests/invariants/central-de-disparos-nucleo.test.ts — dois tenants reais por JWT: leitura positiva do dono e negativa cruzada da outra organização; o navegador só tem SELECT (INSERT/UPDATE/DELETE/TRUNCATE recusados a authenticated e anon); escrita só pelas funções fn_campaign_* do service_role" },
  { tabela: "campaign_destinations", razao: "tests/invariants/central-de-disparos-nucleo.test.ts — dois tenants reais por JWT: leitura positiva do dono e negativa cruzada da outra organização; o navegador só tem SELECT (INSERT/UPDATE/DELETE/TRUNCATE recusados a authenticated e anon); escrita só pelas funções fn_campaign_* do service_role" },
  { tabela: "campaign_channels", razao: "tests/invariants/central-de-disparos-nucleo.test.ts — dois tenants reais por JWT: leitura positiva do dono e negativa cruzada da outra organização; o navegador só tem SELECT (INSERT/UPDATE/DELETE/TRUNCATE recusados a authenticated e anon); escrita só pelas funções fn_campaign_* do service_role" },
  { tabela: "campaign_contacts", razao: "tests/invariants/central-de-disparos-nucleo.test.ts — dois tenants reais por JWT: leitura positiva do dono e negativa cruzada da outra organização; o navegador só tem SELECT (INSERT/UPDATE/DELETE/TRUNCATE recusados a authenticated e anon); escrita só pelas funções fn_campaign_* do service_role" },
  { tabela: "campaign_events", razao: "tests/invariants/central-de-disparos-nucleo.test.ts — dois tenants reais por JWT: leitura positiva do dono e negativa cruzada da outra organização; o navegador só tem SELECT (INSERT/UPDATE/DELETE/TRUNCATE recusados a authenticated e anon); escrita só pelas funções fn_campaign_* do service_role" },
